...
$\forall f \in$ \gls{F}, with \gls{F} representing \textit{the set of all possible analyses}.
...
The role of $L$ is important, in that it is often the derivatives of $L$ that allows us to explore \gls{F}.
Much research in these fields can be seen as concerning itself with 1) developing new methods for exploring \gls{F}, and 2) developing new functions $L$ to facilitate progress with (1).
...
Formally, let us think of a representation $r \in$ \gls{R} as a process applied to the world $Y$, resulting in some objective measurement (data) $X \triangleq r(Y)$.
...
Given two measurements $r_1, r_2 \in$ \gls{R}, if $r_1$ captures more information, then:
...
$g \in$ \gls{G}, which maps representation $X$ to $X^\prime$.
...
$\forall g \in$ \gls{G}. With this result in hand, it may seems like transforming between symbolic representations is pointless.
...
$\forall r \in$ \gls{R},  $\forall f_g \in \mathcal{F}_g$. Ranging across all $g$ and $f$, we see that for each task-specific utility $U$, it may always be possible to develop some new $r^*$ such that:
...
In the context of this argument, we will say that \textit{measurement is more effective when the representation captures accurately any special structure of the aspect of the world being measured}.
